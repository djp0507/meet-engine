...
int64_t g_m3u8_last_keyframe_pts = 0;
int g_m3u8_start_sequence = 0;
...
static int hls_start(AVFormatContext *s)
{
	...
	av_log(s, AV_LOG_INFO,"hls_start:%s\n", oc->filename);
	c->ts_file_count++;
    return 0;
}
...
{
	av_log(s, AV_LOG_INFO,"hls_write_header\n");
    HLSContext *hls = s->priv_data;
    int ret, i;
    char *p;
    ...
    //hls->number      = 0;
    hls->number      = g_m3u8_start_sequence;//hls->sequence;
	hls->ts_file_count = 0;
...
static int hls_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    {
        ret = 0;//append_entry(hls, hls->duration);
        if (ret)
            return ret;
		...
		else if(hls->ts_file_count == 10)
		{
        	g_m3u8_last_keyframe_pts = pkt->pts*AV_TIME_BASE*av_q2d(st->time_base);
			g_m3u8_start_sequence = hls->number -1;
		}
		else
		{
			//do nothing
	...
	if(oc->pb != NULL)
	{
		av_log(s, AV_LOG_INFO,"ff_write_chained:%s\n", oc->filename);
...
{
	av_log(s, AV_LOG_INFO,"hls_write_trailer\n");
    HLSContext *hls = s->priv_data;
    AVFormatContext *oc = hls->avf;
...
static const AVOption options[] = {
    ...
    {"hls_time",      "set segment length in seconds",           OFFSET(time),    AV_OPT_TYPE_FLOAT,  {.dbl = 10},     2, FLT_MAX, E},
    {"hls_list_size", "set maximum number of playlist entries",  OFFSET(size),    AV_OPT_TYPE_INT,    {.i64 = 10000},     0, INT_MAX, E},
