    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        if (var->needed && !var->pkt.data) {
            while (1) {
				...
				//roger
				//skip seeking key frame if packet is from local PPTV SDK as it steams first key frame.
				if(strstr(s->filename, "127.0.0.1"))
				{
        			av_log(NULL, AV_LOG_INFO, "hls_read_packet, s->filename:%s\n", s->filename);
					c->seek_timestamp = AV_NOPTS_VALUE;
					break;
				}
