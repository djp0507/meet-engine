...
static  int                g_cpuFreq;
static  int                g_cpuArchNumber;
...
cpulist_read_from(CpuList* list, const char* filename)
{
    ...
    if (filelen < 0) {
        LOGD("Could not read %s: %s", filename, strerror(errno));
        return;
    }
...
get_cpu_freq(void)
{
	...
	{
		LOGD("file %s len:%d", filename, filelen);
	}
...
android_cpuInit(void)
{
    ...
    LOGD("found cpuCount = %d\n", g_cpuCount);

	g_cpuFreq = get_cpu_freq();
    LOGD("found cpuFreq= %d\n", g_cpuFreq);
...
#ifdef __ARM_ARCH__
    {
        ...
        if (cpuArch != NULL) {
            ...
            LOGD("found cpuArch = '%s'\n", cpuArch);

            /* read the initial decimal number, ignore the rest */
            //archNumber = strtol(cpuArch, &end, 10);
			g_cpuArchNumber = strtol(cpuArch, &end, 10);

            /* Here we assume that ARMv8 will be upwards compatible with v7
                * in the future. Unfortunately, there is no 'Features' field to
                * indicate that Thumb-2 is supported.
                */
            if (end > cpuArch && g_cpuArchNumber >= 7) {
                hasARMv7 = 1;
            }
            ...
            if (hasARMv7) {
                char* cpuProc = extract_cpuinfo_field(cpuinfo, cpuinfo_len,
                                                      "Processor");
                if (cpuProc != NULL) {
                    LOGD("found cpuProc = '%s'\n", cpuProc);
                    if (has_list_item(cpuProc, "(v6l)")) {
                        LOGD("CPU processor and architecture mismatch!!\n");
                        hasARMv7 = 0;
                    }
                    free(cpuProc);
                }
        ...
        if (cpuFeatures != NULL) {
            ...
            if (has_list_item(cpuFeatures, "vfpv3"))
                ...
                g_cpuFeatures |= ANDROID_CPU_ARM_FEATURE_VFPv3d16;
...
int
android_getCpuArchNumber(void)
{
    pthread_once(&g_once, android_cpuInit);
    return g_cpuArchNumber;
}
